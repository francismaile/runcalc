		...
		function calculate(calcWhich) {
			...
			switch(which) {
				...
				case 'time':
					...
					const temp = (toSeconds(pace) * distance);
					console.log(new Date(temp).toISOString() );
					theForm['hours'].value = calcRaceTime.split(':')[0];
					theForm['minutes'].value  = calcRaceTime.split(':')[1];
					theForm['minutes'].value = calcRaceTime.split(':')[2];
					break;
				case 'distance':
